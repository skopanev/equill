...
//! without a network. What cannot be tested here is the socket itself, and
//! that is exactly what this file is: configuration, no decisions.
use super::super::super::super::model::vector_error;
use crate::kernel::error::Error;
use std::time::Duration;
use ureq::Agent;

/// Where a DeepInfra embedding is asked for, verbatim from
/// <https://docs.deepinfra.com/apis/embeddings>. Fixed, not configured: an
/// endpoint an operator can set is an endpoint whoever can write the descriptor
/// can point at themselves, and the key travels with the request.
pub(super) const ENDPOINT: &str = "https://api.deepinfra.com/v1/openai/embeddings";

/// A prompt-time hook can wait behind this. Long enough for a small batch
/// against an 8B model, short enough that a hung call fails instead of sitting
/// in front of an agent's first turn.
const REQUEST_TIMEOUT: Duration = Duration::from_secs(30);

/// A batch of 4096 floats renders well under this. The cap exists so a
/// provider answering with something else — a proxy error page, a stream that
/// does not end — cannot be read into memory unbounded.
...
impl Post for Https {
    fn post_json(&self, key: &str, body: &str) -> Result<(u16, String), Error> {
        ...
        let mut response = match response {
            ...
            Err(_) => return Err(vector_error("deepinfra embedding request failed")),
        };
        ...
        let body = response
            ...
            .limit(MAX_BODY)
            .read_to_string()
            .map_err(|_| vector_error("deepinfra embedding response could not be read"))?;
        Ok((status, body))
    }
}
